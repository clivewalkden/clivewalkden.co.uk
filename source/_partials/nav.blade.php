<nav class="w-full flex-grow lg:flex lg:items-center lg:w-auto hidden lg:block pt-6 lg:pt-0 justify-end" id="nav-content">
    <ul class="list-reset flex justify-between flex-1 md:flex-none items-center">
        <li class="mr-3">
            <a href="/" class="{{ $page->selected('') }}">Home</a>
        </li>
        <li class="mr-3">
            <a href="/portfolio/" class="{{ $page->selected('portfolio') }}">Portfolio</a>
        </li>
        <li class="relative mr-3 group">
            <a href="/" class="{{ $page->selected('code') }}">Code</a>
            <ul class="absolute w-64 left-0 top-0 mt-10 p-2 rounded-lg shadow-lg bg-white z-10 hidden group-hover:block">
                <svg class="block fill-current text-white w-4 h-4 absolute left-0 top-0 ml-3 -mt-3 z-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"></path></svg>
                <li class="p-1 whitespace-no-wrap rounded-full text-sm md:text-base text-gray-600 hover:text-gray-800 hover:bg-gray-100">
                    <a class="px-2 py-1" href="/test">
                        <span class="">CW Hide Reveal</span>
                    </a>
                </li>
                <li class="p-1 whitespace-no-wrap rounded-full text-sm md:text-base text-gray-600 hover:text-gray-800 hover:bg-gray-100">
                    <a class="px-2 py-1" href="/test">
                        <span class="">CW Char Count</span>
                    </a>
                </li>
                <li class="p-1 whitespace-no-wrap rounded-full text-sm md:text-base text-gray-600 hover:text-gray-800 hover:bg-gray-100">
                    <a class="px-2 py-1" href="/test">
                        <span class="">CW Abandon Mouse Tracker</span>
                    </a>
                </li>
                <li class="p-1 whitespace-no-wrap rounded-full text-sm md:text-base text-gray-600 hover:text-gray-800 hover:bg-gray-100">
                    <a class="px-2 py-1" href="/test">
                        <span class="">CW Event Tracking</span>
                    </a>
                </li>
                <li class="p-1 whitespace-no-wrap rounded-full text-sm md:text-base text-gray-600 hover:text-gray-800 hover:bg-gray-100">
                    <a class="px-2 py-1" href="/test">
                        <span class="">CW Alert</span>
                    </a>
                </li>
            </ul>
        </li>
        <li class="fmr-3">
            <a href="/blog/" class="{{ $page->selected('blog') }}">Blog</a>
        </li>
        <li class="mr-3">
            <a href="/about/" class="{{ $page->selected('about') }}">About</a>
        </li>
    </ul>
</nav>