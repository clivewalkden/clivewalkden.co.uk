    ...
    <div class="px-4 py-16 mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-8 lg:py-20">
        ...
        <div class="text-center">
            <p class="text-sm md:text-base text-yellow-500 font-bold">
                <time datetime="{{ date('Y-m-d\TH:i:sP', $page->published) }}">{{ date('j F Y', $page->published) }}</time> <span class="text-gray-900">/</span> <a href="{{ $category->getPath() }}">{{ $category->title }}</a>
            </p>
            <div class="flex">
                @if ($page->getPrevious())
                    <a href="{{ $page->getPrevious()->getPath() }}" title="{{ $page->getPrevious()->title }}" class="flex-none text-sm self-center">Previous Article</a>
                @endif
                ...
                @if ($page->getNext())
                    <a href="{{ $page->getNext()->getPath() }}" title="{{ $page->getNext()->title }}" class="flex-none text-sm self-center">Next Article</a>
                @endif
            </div>
        </div>
